...
const DBNewItem = () => {
        ...
        uploadTask.on('state_changed',
            ...
            () => {
                getDownloadURL(uploadTask.snapshot.ref).then((downloadURL) => {
                    ...
                    setIsLoading(false);
                    setProgress(null);
                    dispatch(alertSuccess("Image is uploaded to the cloud"));
                    setTimeout(() => {
                        dispatch(alertNull());
                    }, 3000);
                });
    ...
    const deleteImageFromFirebase = () => {
        ...
        deleteObject(deleteRef).then(() => {
            ...
            setIsLoading(false)
            dispatch(alertSuccess("Image is removed from the cloud"));
            setTimeout(() => {
                dispatch(alertNull());
            }, 3000);
        })
            ...
            <div className=' border border-gray-300 rounded-md p-4 w-full flex flex-col items-center justify-center gap-4'>
                ...
                <div className=" w-full flex items-center justify-around gap-3 flex-wrap">
                    {statuses && statuses?.map(data => (
                        <p key={data.id} onClick={() => setCategory(data.category)} className={` px-4 py-3 rounded-md text-xl text-textColor font-semibold cursor-pointer hover:shadow-md border border-gray-200 backdrop-blur-md ${data.category === category ? ' bg-red-400 text-primary' : ' bg-transparent'}`}>
                            {data.title}
                        </p>
                    ))}
                </div>
